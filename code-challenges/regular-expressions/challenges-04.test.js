...
const isCapitalized = (str) => {
  ...
  let finalArray = [];
  let punctuationRemoval = /[.,!)]/g
  for (let i = 0; i < stringArray.length; i++) {
      if (regex.test(stringArray[i])) {
        let input = stringArray[i].replace(punctuationRemoval, '')
...
const citiesAtoJ = (arr) => {
    let regex = /^[A-J]///g WHY DOES THIS BREAK?
    let finalArray = [];
...
const noPunctuation = str => {
  ...
  let finalArray = [];
  for (let i = 0; i < stringArray.length; i++) {
      if (!hasPunctuation.test(stringArray[i])) {
...
const findShells = (str) => {
  ...
  for (let i = 0; i < splitString.length; i++) {
    if (regex.test(splitString[i])) {
      ...
      let input = splitString[i].replace(punctuationRemoval, '')
      ellArray.push(input)
    }
  }
  console.log(ellArray)
  return ellArray;
};
