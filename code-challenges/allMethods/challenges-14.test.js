...
const toTitleCase = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    newArray.push(arr[i].slice(0,1).toUpperCase() + arr[i].slice(1, arr[i].length))
  }
  return newArray;
};
...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (parseInt(arr[i].mass) > parseInt(arr[0].mass)) {
        biggerCharacters.push(arr[i])
    }
  }
  if (biggerCharacters.length > 1) {
    let bigString = biggerCharacters[0].name
    for(let j = 1; j < biggerCharacters.length; j++) {
        bigString = bigString + " - " + biggerCharacters[j].name
    }
    return bigString;
  } else {
      return ""
...
const detectTicTacToeWin = (board) => {
  let winConditions = [[0,0,0,1,0,2], [1,0,1,1,1,2], [2,0,2,1,2,2], [0,0,1,0,2,0], [0,1,1,1,2,0], [0,2,1,2,2,2], [0,0,1,1,2,2], [0,2,1,1,2,0]]
  for (let i = 0; i < winConditions.length; i++) {
    if (helperCheck(board, winConditions[i][0], winConditions[i][1], winConditions[i][2], winConditions[i][3], winConditions[i][4], winConditions[i][5])) {
      return true;
    }
  }
  return false;
...
const helperCheck = (board, row1, col1, row2, col2, row3, col3) => {
