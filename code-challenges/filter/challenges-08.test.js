...
const oddValues = (arr) => {
    const returnArray = arr.filter(num => {
        if (num % 2 != 0) {
            return num
        }
    });
...
const filterStringsWithVowels = (arr) => {
    const returnArray = arr.filter(string => {
        let regex = /[aeiou]/
...
const getStatName = (arr, minBaseStat) => {
    const returnArray = arr.filter(value => {
        if (value.baseStat > minBaseStat) {
            //FRUSTRATING IT LOOKS LIKE ITS ADDING THE NAME BUT IT ACTUALLY ADDS THE WHOLE VALUE
...
const getCharactersWithoutChildren = (arr) => {
    const returnArray = arr.filter(person => {
...
const evenOddNumericValues = (arr) => {
    ...
    const evenOddMap = filteredArray.map(num => {
        if (num % 2 == 0) {
            return 'even'
        } else {
            return 'odd'
