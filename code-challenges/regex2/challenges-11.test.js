...
const validateEmail = (email) => {
  let regex = /^[A-z0-9]+(.[A-z0-9]+)?@[A-z0-9]+.(com|net|org)+$/g
...
const validatePhoneNumber = (phoneNumber) => {
    // let regex = /(^\d{3}\d{3}\d{4}$)|(^\(\d{3}\)\s\d{3}-\d{4}$)|(^\d{3}\s\d{3}\s\d{4}$)|(^\(\d{3}\)\d{3}\s\d{4}$)|(^\d{3}\s\d{3}-\d{4}$)|(^\d{3}-\d{3}\d{4}$)|(^\d{3}-\d{3}\s\d{4}$)|(^\d{3}-\d{3}-\d{4}$)|(^\d{3}\d{3}-\d{4}$)|(^\d{3}\s\d{3}\d{4}$)/g
    let regex = /(^\d{3}\s?-?\d{3}\s?-?\d{4}$)|(^\(\d{3}\)\s?\d{3}\s?-?\d{4}$)/
    return regex.test(phoneNumber); //WHYYYYYYYY
};
...
const findTagNames = elements => {
  ...
  elements.forEach(tag => {
    tagArray.push(tag.match(regex))
  })
  tagArray = [].concat.apply([], tagArray);
  return tagArray
};
