...
const firstLetters = (arr) => {
    let firstLetters = [];
  arr.forEach(str => {
    firstLetters.push(str.slice(0,1))
  })
  return firstLetters;
};
...
const findHappiness = (arr) => {
  let happyStrings = arr.reduce( (accumulator, str, idx) => {
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.forEach(phoneNumber => {
      ...
      for(let i = 0; i < phoneNumber.length; i++) {
        if (phoneNumber.slice(i, i+1) !== "(" & phoneNumber.slice(i, i+1) !== ")" & phoneNumber.slice(i, i+1) !== " " & phoneNumber.slice(i, i+1) !== "-") {
            fixedNumber = fixedNumber + phoneNumber.slice(i, i+1)
        }
      }
...
const findAnything = (arr, target) => {
  ...
  for (let i = 0; i < arr.length; i ++) {
      let success = false;
      for (let j = 0; j < arr[i].length - target.length + 1; j++) {
        if (arr[i].slice(j, j + target.length) === target) {
            success = true;
        }
      }
      if (success) {
...
const findEvery = (arr, target) => {
    for (let i = 0; i < arr.length; i++) {
        let hasHappy = false;
        for (let j = 0; j < arr[i].length - target.length; j++) {
            if (arr[i].slice(j, j + target.length) === target) {
                hasHappy = true;
                break;
            }
...
const characterByIndex = (arr) => {
    let newArray = [];
  for(let i = 0; i < arr.length; i++) {
    newArray.push(arr[i].slice(i, i+1))
  }
  return newArray;
};
