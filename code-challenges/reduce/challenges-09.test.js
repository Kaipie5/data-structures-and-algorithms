...
const reversedString = (str) => {
    ...
    let reversedString = letterArray.reduce( (accumulator, person, idx) => {
        accumulator = accumulator + letterArray[str.length - idx -1]
        return accumulator
    }, "");
...
const countNumberOfChildren = (arr) => {
    let children = arr.reduce( (accumulator, person, idx) => {
...
const countPrimeNumbers = (arr) => {
  let numPrimes = arr.reduce( (accumulator, value, idx) => {
    if (isPrime(value)) {
      accumulator = accumulator + 1
    }
    return accumulator
...
const extractStat = (statName, arr) => {
  let stats = arr.reduce( (accumulator, value, idx) => {
...
const extractChildren = (arr) => {
  ...
  let aPeople = arr.filter(person => {
    if (person.name.includes('a')) {
      return person
    }
  });
  let allAPeople = aPeople.reduce( (accumulator, person, idx) => {
    if (person.children != null) {
      for (let i = 0; i < person.children.length; i++) {
          accumulator.push(person.children[i])
      }
    }
      return accumulator
  }, []);
  console.log(allAPeople)
  return allAPeople;
};
